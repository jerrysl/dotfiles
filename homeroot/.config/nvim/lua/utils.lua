...
-- Cleaner way of using diagnostic api. However it doesn't
-- strictly toggle virtual_text. It toggles all diagnostics, so
-- no more sign column or underlining when disabled.

-- I could parameterize this with local/global toggles but I don't
-- think I need it.
M.toggle_diagnostic = function()
  -- local global_state = vim.diagnostic.is_enabled({ bufnr = nil })
  local buf_state = vim.diagnostic.is_enabled({ bufnr = 0 })
  vim.diagnostic.enable(not buf_state)
  if buf_state == true then
...
-- Awkward virtual_text-only function from my old config
